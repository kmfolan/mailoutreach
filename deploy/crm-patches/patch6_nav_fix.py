#!/usr/bin/env python3
"""Robust CRM nav link fix — uses regex, not exact-string match"""
import re, sys
...
'''
    # Find the admin section and insert before it
    admin_match = re.search(r'<section[^>]*id="admin"', src)
    if admin_match:
        pos = admin_match.start()
        # Back up to find the start of the line
        line_start = src.rfind('\n', 0, pos) + 1
        src = src[:line_start] + crm_html + src[line_start:]
        print("  + CRM section + modals added")
        changed = True
    else:
        print("  ! Could not find admin section")
        sys.exit(1)
else:
...
if changed:
    ...
    print("index.html already up to date — no changes needed")
